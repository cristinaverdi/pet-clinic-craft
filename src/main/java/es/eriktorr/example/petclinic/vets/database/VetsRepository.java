...
public class VetsRepository {
    ...
    private static final String LIST_ALL_VETERINARIANS_SQL =
            "with t1 as (" +
    ...
    public List<Vet> listAll() {
        return entitiesGroupedById().entrySet().stream().map(entity -> {
            val entities = entity.getValue();
    ...
    private Map<Integer, List<VetEntity>> entitiesGroupedById() {
        return jdbcTemplate.query(LIST_ALL_VETERINARIANS_SQL, new VetEntityRowMapper()).stream()
